...
MQ.TileLayer = L.TileLayer.extend({
    ...
    setMapType: function(D) {
        if (D != this.options.mapType) {
            ...
            if (C) {
                this._url = C.url;
                this._openUrl = C.openUrl;
                L.setOptions(this, C);
                this.redraw()
            }
            this.fire("maptypechange", {layer: this})
        }
    },
    setUrl: function(C, D) {},
    onAdd: function(B) {
    ...
    onRemove: function(D) {
        if (D.mapquest.layers.length == 1) {
            D.removeControl(D.mapquest.logo);
            D.removeControl(D.mapquest.attributionControl);
            D.mapquest.layers = [];
    ...
    getTileUrl: function(F) {
        var E = new Boundary(boundaryDefinition);
        if (this._url) {
            if (this._adjustTilePoint) {
                this._adjustTilePoint(F)
            }
            var D = this._openUrl;
            if (MQCONFIGNUMBER !== 4 && E.contains(F.z, F.x, F.y)) {
                D = this._url
            }
            return L.Util.template(D, L.extend({
    ...
    _loadTransactionImage: function(N, S, Q, R, K) {
        ...
        var T = "L_" + L.version + "_" + MQ.mapConfig.getConfig("version") + "_" + (MQ.mapConfig.getConfig("configNumber") == "4"
            ? "OSM"
            : "MQ");
...
function _tile2long(C, D) {
    C = C + 0.5;
    return (C / Math.pow(2, D) * 360 - 180)
}
function _tile2lat(E, D) {
    E = E + 0.5;
    var F = Math.PI - 2 * Math.PI * E / Math.pow(2, D);
    return (180 / Math.PI * Math.atan(0.5 * (Math.exp(F) - Math.exp(-F))))
}
function Boundary(F) {
    var G = [];
    for (var H = 0; H < F.features.length; H++) {
        var E = F.features[H].geometry.coordinates[0];
        G.push(new GeoPolygon(E, F.features[H].properties))
    }
    this.polies = G
}
Boundary.prototype = {
    contains: function(J, H, I) {
        if (J <= 5) {
            return false
        }
        var K = _tile2long(H, J);
        var N = _tile2lat(I, J);
        var O = false;
        for (var M = 0; M < this.polies.length; M++) {
            if (this.polies[M].contains(K, N, J)) {
                O = true;
                break
            }
        }
    ...
    containsLngLat: function(H, J, G) {
        if (H <= 5) {
            return false
        }
        var F = false;
        for (var I = 0; I < this.polies.length; I++) {
            if (this.polies[I].contains(J, G, H)) {
                F = true;
                break
            }
        }
...
function GeoPolygon(D, C) {
    this.latLngArray = D;
    this.minZoom = C.minZoom || 1
}
GeoPolygon.prototype = {
    contains: function(N, I, K) {
        if (K < this.minZoom) {
            return false
        }
        var H = this.latLngArray.length;
        var M,
            O = H - 1;
        var J = false;
        for (M = 0; M < H; M++) {
            if ((this.latLngArray[M][1] < I && this.latLngArray[O][1] >= I || this.latLngArray[O][1] < I && this.latLngArray[M][1] >= I) && (this.latLngArray[M][0] <= N || this.latLngArray[O][0] <= N)) {
                if (this.latLngArray[M][0] + (I - this.latLngArray[M][1]) / (this.latLngArray[O][1] - this.latLngArray[M][1]) * (this.latLngArray[O][0] - this.latLngArray[M][0]) < N) {
                    J = !J
                }
            }
            O = M
...
MQ.Util = L.Class.extend({
    ...
    quote: function(B) {
        ...
        return this.escapable.test(B)
            ? '"' + B.replace(this.escapable, function(D) {
                var A = this.meta[D];
                return typeof A === "string"
                    ? A
                    : "\\u" + ("0000" + D.charCodeAt(0).toString(16)).slice(-4)
            }) + '"'
            : '"' + B + '"'
    },
    str: function(P, S) {
...
MQ.MapConfig = L.Class.extend({
    ...
    _parseHardCoded: function() {
        ...
        this._config.iconCDN = MQICONCDN;
        this._config.map = {
            url: this._leafletURL(MBMAP),
            openUrl: this._leafletURL(MBMAP_OPEN),
            ext: MBEXT,
            subdomains: this._mapBoxSubDomains(MBTILELO, MBTILEHI)
        };
        this._config.hyb = {
            url: this._leafletURL(MBHYB),
            openUrl: this._leafletURL(MBHYB_OPEN),
            ext: MBEXT,
            subdomains: this._mapBoxSubDomains(MBTILELO, MBTILEHI)
        };
        this._config.sat = {
            url: this._leafletURL(MBSAT),
            openUrl: this._leafletURL(MBSAT),
            ext: MBEXT,
            subdomains: this._mapBoxSubDomains(MBTILELO, MBTILEHI)
        };
        this._config.light = {
            url: this._leafletURL(MBLIGHT),
            openUrl: this._leafletURL(MBLIGHT_OPEN),
            ext: MBEXT,
            subdomains: this._mapBoxSubDomains(MBTILELO, MBTILEHI)
        };
        this._config.dark = {
            url: this._leafletURL(MBDARK),
            openUrl: this._leafletURL(MBDARK_OPEN),
            ext: MBEXT,
            subdomains: this._mapBoxSubDomains(MBTILELO, MBTILEHI)
        };
        this._config.copyrightServer = MBCOPYRIGHT + MBCOPYRIGHT_PATH;
        this._hasLoaded = 2;
        this.fire("load", this)
    },
    _leafletURL: function(B) {
        if (B) {
            ...
            B = B.replace(".{ext}", this._config.hd + ".{ext}")
        }
        return B
    },
    _mapBoxSubDomains: function(I, F) {
        var J = ["a", "b", "c", "d"];
        var G = [];
        for (var H = I; H <= F; H++) {
            G.push(J[H - 1])
    ...
    _mapquestSubdomains: function(H, E) {
        var F = [];
        for (var G = H; G <= E; G++) {
            F.push(G)
        }
...
MQ.Control.Attribution = L.Control.extend({
    ...
    refreshAttribution: function() {
        ...
        var Q,
            ...
            W,
            Y = {},
            N = window.location.hostname.match(/.ca$/)
        ...
        O.sort(COPYRIGHT_SORT);
        this.list = O;
        for (X = 0; X < O.length; X++) {
            Q = O[X][0];
            R = O[X][1];
            if (R != undefined && (R.text != "" || R.html != "") && X == 0) {
                P.push(",&nbsp;")
            }
            if (!R || Y[Q]) {
                continue
            ...
            if (R.html) {
                switch (R.text_short) {
                    case "MB":
                        P.push('<a href="http://hello.mapquest.com/attributions-mapbox">&copy; Mapbox</a>');
                        break;
                    case "OSM":
                        P.push('<a href="http://www.openstreetmap.org/about">' + R.html + "</a>");
                        break;
                    default:
                        P.push(this.replaceHtml(R.html))
                }
            } else {
                if (R.text) {
                    P.push(this.escapeHtml(R.text))
                }
            }
        }
        termsHTML = '<a id="terms" class="termsLink" target="_blank" href="' + N + '">Terms</a>';
    ...
    refreshCoverage: function() {
        ...
        var J = (this._currentBounds._northEast.lat + this._currentBounds._southWest.lat) / 2;
        var M = (this._currentBounds._northEast.lng + this._currentBounds._southWest.lng) / 2;
        var I = {
            html_short: "",
            html: "&copy; Mapbox",
            text_short: "MB",
        ...
        var K = {
            html_short: "",
            html: "&copy; OpenStreetMap",
            text_short: "OSM",
            text: "OpenStreetMap",
            id: "osm"
        };
        var H = new Boundary(boundaryDefinition);
        this.list = [];
        if (this._currentMapType !== "sat") {
            if (MQCONFIGNUMBER !== 4 && H.containsLngLat(this._currentZoom, M, J)) {
                this.set("tt", G)
            } else {
                this.set("osm", K)
            }
...
MQ.util.loadCSS(".mq-logo-control{margin-left: 2px !important;margin-bottom: 5px !important;}.mq-logo-control img{ position: relative;}");
MQ.Control.Logo = L.Control.extend({
    ...
    onAdd: function(B) {
        ...
        this._container.innerHTML = '<img src="' + MQ.mapConfig.getConfig("imagePath") + 'questy_sm.png" width="100" height="20"/>';
        return this._container
    }
});
